...
function DropDownMenu_GetHandle() return {
	...
	SetMenuRequestFunc = function(self, object, func)
		self.menuRequestObject = nil 
		self.menuRequestFunc = nil
...
--------------------------------------------------------------------------
local DROPDOWNMENU_GUID = "{FA9471E6-1E60-4D7A-B07C-7B2119DFDD91}" -- Library guid
local DROPDOWNMENU_INSET_HORIZONTAL = 12 -- Menu horizontal edge size
local DROPDOWNMENU_INSET_VERTICAL = 12 -- Menu vertical edge size
local DROPDOWNMENU_BUTTON_HEIGHT = 18 -- Menu button height
local DROPDOWNMENU_HEADER_HEIGHT = 20 -- Menu header height
local DROPDOWNMENU_BUTTON_WIDTH_MIN = 160 -- Menu button minimum width
...
local function MenuButton_OnClick(self)
	...
	if info.hasConfirm then
		...
		StaticPopup_Show(DROPDOWNMENU_GUID)
	else
		SafeCall(info)
	end
...
local function MenuButton_SetText(self, id, text, justifyH, r, g, b, colorCode, isTitle, disabled)
	...
	return math.ceil(fs:GetWidth()) + DROPDOWNMENU_TEXT_INSET, text
end
...
local function MenuButton_SetInfo(self, info, header)
	...
	local height = info.hasContents and DROPDOWNMENU_BUTTON_HEIGHT or DROPDOWNMENU_SEPARATOR_HEIGHT
...
local function MenuButton_AlignText(self, fs, left, width, isColumn)
	...
	if justifyH == "RIGHT" then
		fs:SetPoint("RIGHT", self, "LEFT", left + width - spacing, 0)
	elseif justifyH == "CENTER" then
		fs:SetPoint("CENTER", self, "LEFT", left + (width - spacing) / 2, 0)
	else
		fs:SetPoint("LEFT", self, "LEFT", left, 0)
	end
...
local function SetFrameAttributeMenu(frame)
	...
	frame.IsCounting = function(self) 
...
local function CreateDropDownMenu(level)
	...
	local frame = CreateFrame("Frame", DROPDOWNMENU_NAME_PREFIX.."Level"..level, GetDropDownMenu(level - 1) or UIParent, "BackdropTemplate")
	...
	frame.Open = function(self, owner, value, button)
		...
		owner.headerList= nil
		owner.infoList= nil 
		owner.hasContents = nil
		...
		self:SetHeight(headerHeight + listHeight + DROPDOWNMENU_INSET_VERTICAL * 2)
		self:Show()
...
local sllder = CreateFrame("Button", "WowBeeSister",UIParent, "SecureActionButtonTemplate");
root.highlightTexture = highlightTexture
highlightTexture:Hide()
highlightTexture:SetTexture("Interface\\QuestFrame\\UI-QuestTitleHighlight")
highlightTexture:SetBlendMode("ADD")
highlightTexture:SetVertexColor(1, 1, 1, 0.7)
...
local function InitializeDropDownMenuSubControl(frame, subControlType)
	...
	if not editbox then
		...
		local edge = CreateFrame("Frame", nil, editbox, "BackdropTemplate")
...
local editFrame = GetEditBoxMenu() or CreateFrame("Frame", DROPDOWNMENU_NAME_PREFIX.."EditBoxFrame",UIParent,"BackdropTemplate")
...
local sliderFrame = GetSliderMenu() or CreateFrame("Frame", DROPDOWNMENU_NAME_PREFIX.."SliderFrame",UIParent,"BackdropTemplate")

InitializeDropDownMenuSubControl(sliderFrame, "Slider")
sllder:SetAttribute("type", "macro") ;
...
local slider = sliderFrame.childSlider or CreateFrame("Slider", nil, sliderFrame,"BackdropTemplate")
...
SetOverrideBindingClick(sllder, true, "F12" , sllder:GetName())	
