	...
	function Lib.CreateMain(name,parent,text)
		...
		local title = CreateFrame("Frame", "$parent_Caption", frame,"BackdropTemplate")
	...
	function Lib.CreateEditer(name,parent,ref)
		...
		local scrollBackground = CreateFrame("Frame", "$parent_CodeBackground", frame,"BackdropTemplate");
...
--[[	frame.DDEdit:SetScript("OnEnterPressed",function(self)
	...
	--DropdownList Frame
	frame.DDList=CreateFrame("Frame",name .. "List",frame,"BackdropTemplate");
...
local icon=1;gsub(icon, "[%l%u]+\\", "");
	...
	if(not MacroPopupFrame.Bak)then
		...
		ShowUIPanel(MacroPopupFrame)
		
		-- local kids = {MacroPopupFrame:GetRegions()}
		...
		f.OnOkayClick=MacroPopupFrame.BorderBox.OkayButton:GetScript("OnClick");
	...
	MacroPopupEditBox:SetScript("OnEnterPressed", function(self)
		...
		MacroPopupFrame:OnOkay(MacroPopupEditBox:GetText(),getglobal("MacroPopupButton"..(MacroPopupFrame.selectedIcon%80).."Icon"):GetTexture());
	end);
	MacroPopupFrame.BorderBox.OkayButton:SetScript("OnClick",function(self)
	...
	MacroPopupFrame:HookScript("OnHide", function(self)
		...
		MacroPopupFrame.BorderBox.OkayButton:SetScript("OnClick",MacroPopupFrame.Bak.OnOkayClick);
