...
function BeeSpellIsAllClassInf(SpellIds,NoSpellTarget,BuffNames)
	...
	NoSpellTarget[SpellIds["移动银行"]] = true;
	NoSpellTarget[SpellIds["熔炼"]] = true;
	NoSpellTarget[SpellIds["组团出发"]] = true;
	NoSpellTarget[SpellIds["群体复活"]] = true;
...
IgnoreSpellIds["移动银行"]=83958;
IgnoreSpellIds["熔炼"]=2656;
IgnoreSpellIds["组团出发"]=83967;
IgnoreSpellIds["群体复活"]=83968;
...
function BeeSpellIsMoveAll(Id,unit,name) --处理移动中能施放的技能
	if WowUnitBuff("player", N_97128, 1) then 
		return true;
	end
end


function BeeGetSpellIdEx(spell)
	return SpellIdsEx[spell] or IgnoreSpellIds[spell]
end