function BeeIsRun(spell, unit, gcd, special, isRun, NOCD, EnergyDemand) -- 是否可以对此目标施放技能
    ...
    -- print(GetTime(),spell,A, B, C, D, E, F,unit, I)
...
function BeeGCD(spellname) -- 获得某职业的公共CD
    local spellid;
    if spellname then
        _, spellid = GetSpellBookItemInfo(spellname)
        if not spellid then
            spellid = BeeGetSpellIdEx(spellname)
        end
    else
...
function BeeRange(unit1, unit2) -- 判断距离
    if not unit2 then
        ...
        elseif UnitInParty("player") then
            if (string.lower(string.sub(unit1, 1, 5)) ~= "party" and string.lower(unit1) ~= "player") or
                (string.lower(string.sub(unit2, 1, 5)) ~= "party" and string.lower(unit2) ~= "player") then
        ...
        if mapheight and mapheight > 0 and mapwidth and mapwidth > 0 and unit1x and unit1x > 0 and unit1y and unit1y > 0 and
            unit2x and unit2x > 0 and unit2y and unit2y > 0 then
            local length = math.ceil(math.sqrt(math.pow((unit1x - unit2x) * mapwidth, 2) +
                                                   math.pow((unit1y - unit2y) * mapheight, 2)));
            return length;
        else
            return 100000000;
        end
...
function BeeTalentInfo(talentname) -- 獲得你的天賦某選項的信息
    ...
    for tier = 1, MAX_TALENT_TIERS do
        for column = 1, NUM_TALENT_COLUMNS do
            local talent_id, name, icon, sel, _, spell_id = GetTalentInfo(tier, column, 1)
...
function BeeUnitCastSpellName(unit, interrupt, times) -- 获得指定目标正在施放的法术名称,Interrupt 为非0 只返回可以打断的技能
    unit = BeeUnit(unit, "target");
    local c, _, _, startTime, _, _, _, i = UnitCastingInfo(unit);
    ...
    if c then
        ...
        c, _, _, startTime, _, _, i = UnitChannelInfo(unit);
        if c then
            -- print(GetTime() - startTime/1000,WowBee.Config.ACTime)
            if WowBee.Config.Arena.IsUnitCastSpellName and BeeIsArena() then
                times = IF(times, times, WowBee.Config.ACTime);
...
function BeeUnitCastSpellTime(unit) -- 获得指定目标正在施放的法术剩馀时间
    ...
    local spellch, _, _, startTime, endTimech = UnitChannelInfo(unit)
    if spellch then
...
function BeeUnitCastSpellPassTime(spell, unit) -- 获得指定单位指定技能施放时间.
    ...
    if not (UnitName(unit) or spell) then
        return;
    end
...
function BeeUnitUnitIsPlayer(id) -- 目标的目标是否自己
    ...
    elseif id == 2 then
        return IF(not UnitIsUnit("player", "targettarget") and UnitCanAttack("player", "target") and
                      UnitName("targettarget"), true, false)
    end
end
...
function BeeUnitBuffList(unit, t) -- 获得指定目标buff列表
    ...
    local name = {};
    local i, f, k;
    local c, rank, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable, shouldConsolidate,
        spellId;
    k = 1;

    if WowBee.Config.Arena.BuffList and BeeIsArena() and WowBee.Config.Arena.BuffListTime and
        WowBee.Config.Arena.BuffListTime > 0 and t == 0 then
    ...
    for f = 0, 1 do
        for i = 1, MAX_TARGET_BUFFS do
            if (f == 0) then
                c, rank, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable, shouldConsolidate, spellId =
                    UnitBuff(unit, i);
            else
                c, rank, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable, shouldConsolidate, spellId =
                    UnitDebuff(unit, i);
...
function BeeUnitBuffInfo(Unit, Nameid, BuffType, Categories) -- 获得指定目标buff数量及信息
    ...
    for i = 1, 40 do
        if Categories == 1 then
            name, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable = UnitBuff(Unit, i)
        elseif Categories == 0 then
            name, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable = UnitDebuff(Unit, i)
        end
...
-- spell 法术名
-- unit 对象:target,player
-- nameid 0:施法是player,1:施法是其他,2:没限定
-- buffType 0:没指定,1:Buff,2:Debuff 
function BeeUnitBuff(spell, unit, nameid, buffType, iconName) -- 获得指定目标buff剩馀时间

    unit = BeeUnit(unit, "player");
    nameid = IF(nameid, nameid, 0); -- 默认自己施法
    ...
    if buffType == 0 then
        name, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable = WowUnitBuff(unit, spell,
            nameid)
        if not name then
            name, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable = WowUnitDebuff(unit,
                spell, nameid)
        end
    ...
    elseif buffType == 2 then
        name, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable = WowUnitDebuff(unit, spell,
            nameid)
    end
    ...
    if name then
        ...
        if nameid == 0 and unitCaster == "player" then
            return n, 0, count, debuffType, icon, unitCaster, duration, expirationTime, isStealable;
        elseif nameid == 1 and unitCaster ~= "player" then
            return n, 0, count, debuffType, icon, unitCaster, duration, expirationTime, isStealable;
        elseif nameid == 2 then
            return n, 0, count, debuffType, icon, unitCaster, duration, expirationTime, isStealable;
...
function WowUnitBuff(unit, spell, nameid)
    local nameR, iconR, countR, debuffTypeR, durationR, expirationTimeR, unitCasterR, isStealableR;
    AuraUtil.ForEachAura(unit, "HELPFUL", nil, function(...)
        local name, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable = ...;
        if spell == name and nameid == 0 and unitCaster == "player" then
            nameR, iconR, countR, debuffTypeR, durationR, expirationTimeR, unitCasterR, isStealableR = ...;
            return true;
        elseif spell == name and nameid == 1 and unitCaster ~= "player" then
            nameR, iconR, countR, debuffTypeR, durationR, expirationTimeR, unitCasterR, isStealableR = ...;
            return true;
        elseif spell == name and nameid == 2 then
            nameR, iconR, countR, debuffTypeR, durationR, expirationTimeR, unitCasterR, isStealableR = ...;
            return true;
        end
    end);
    return nameR, iconR, countR, debuffTypeR, durationR, expirationTimeR, unitCasterR, isStealableR;
end

function WowUnitDebuff(unit, spell, nameid)
    local nameR, iconR, countR, debuffTypeR, durationR, expirationTimeR, unitCasterR, isStealableR;
...
function BeeGetSpellCount(spell) -- 获得技能充能层数
...
function BeeTotemType(Type) -- 图腾类型
    if type(Type) ~= "number" then
        WowBee_Message(WowBee.Colors.RED .. "错误：" .. WowBee.Colors.LGREEN ..
                           "参数类型错误，请使用整数值");
        return nil, -1
    end
...
function BeePartyScript(String) -- 獲得符合條件的小隊人物信息--UnitGUID
    ...
    local str = 'function TEMP_BeeParty(name,class,race,spell,unit,guid,spellcd) if ' .. String ..
...
function BeeRaidScript(String) -- 獲得符合條件的團隊人物信息--UnitGUID
    ...
    local str = 'function TEMP_BeeRaid(name,class,race,spell,unit,guid,spellcd) if ' .. String ..
                    ' then return true; else return false; end end'
...
function BeePartyPetScript(String) -- 獲得符合條件的小隊寵物信息
    ...
    local str = 'function TEMP_BeePartyPet(name,class,race,spell,unit,guid,spellcd) if ' .. String ..
                    ' then return true; else return false; end end'
...
function BeeRaidPetScript(String) -- 獲得符合條件的團隊寵物信息
    ...
    local str = 'function TEMP_BeeRaidPet(name,class,race,spell,unit,guid,spellcd) if ' .. String ..
                    ' then return true; else return false; end end'
...
function BeeGroupMinScript(String, strReturn, group) -- 小队或者团队里最小的数值的人物信息
    ...
    if not (group == "party" or group == "partypet" or group == "raid" or group == "raidpet" or group == "arena" or
        group == "arenapet") then
    ...
    local str = 'function TEMP_BeeGroupMin(name,class,race,spell,unit,guid,spellcd) if ' .. String .. ' then return ' ..
                    strReturn .. '; else return false; end end'
...
function BeeGroupMaxScript(String, StrReturn, group) -- 小队或者团队里最大的数值的人物信息
    ...
    local str = 'function TEMP_BeeGroupMax(name,class,race,spell,unit,guid,spellcd) if ' .. String .. ' then return ' ..
                    StrReturn .. '; else return false; end end'
...
function BeeGroupMinFastScript(String, StrReturn, group) -- 小队或者团队里最小的数值的人物信息
    ...
    local str = 'function TEMP_BeeGroupMinFast(name,class,race,spell,unit,guid,spellcd) if ' .. String ..
                    ' then return ' .. StrReturn .. '; else return false; end end'
...
function BeeGroupCountScript(String, StrReturn, group) -- 小队或者团队里符合条件的人物信息数量
    ...
    local str =
        'function TEMP_BeeGroupCount(name,class,race,spell,unit,guid,spellcd) if ' .. String .. ' then return ' ..
            StrReturn .. '; else return false; end end'
...
function BeeGroupMaxTargetScript(String, StrReturn, group) ---、
    ...
    local str = 'function TEMP_BeeGroupMaxTarget(name,class,race,spell,unit,guid,spellcd) if ' .. String ..
                    ' then return ' .. StrReturn .. '; else return false; end end'
    RunScript(str);
...
function BeeGroupMinHealthCast(DeBuffs, Buff, BuffOperator, BuffCd, Health, HealthOperator, Spell, Group)
    ...
    if not str then
        ...
        local text = 'BeeIsRun("' .. Spell ..
                         '",unit) and UnitIsConnected(unit) and not UnitIsCorpse(unit) and not UnitIsDeadOrGhost(unit)';
...
function BeeArenaCastScript(String) -- 獲得競技場敵方正在施法狀態及人物信息
    ...
    local str = 'function TEMP_BeeArenaCast(name,class,race,spell,unit,guid,spellcd) if ' .. String ..
                    ' then return true; else return false; end end'
...
function BeeArenaInfoScript(String) -- 【競技場專用】獲得敵方符合條件的人物信息--UnitGUID
    ...
    local str = 'function TEMP_BeeArena(name,class,race,spell,unit,guid,spellcd) if ' .. String ..
                    ' then return true; else return false; end end'
    ...
    if BeeIsArena() then
        for i = 1, 5 do
            ...
            if IsUnitName_1 and IsUnitName and UnitCanAssist("player", Target) and IsPlayer then
                if TargetClass then
                    isClass = BeeStringFind(TargetClass, BeeUnitClassBase(Target_1)) or
                                  BeeStringFind(TargetClass, BeeUnitClass(Target_1));
                end
        ...
        for i = 1, 5 do
            ...
            if arenaName and arenaTargetName then
                ...
                if acCd > 0 and isUnit then
                    ...
                    if TargetClass then
                        isClass = BeeStringFind(targetClass, BeeUnitClassBase(arenaTarget)) or
                                      BeeStringFind(targetClass, BeeUnitClass(arenaTarget));
    ...
    for f = 0, 1 do
        ...
        for i = 1, MAX_TARGET_BUFFS do
            ...
            if c then
                ...
                WowBee_Message(WowBee.Colors.YELLOW .. "   shouldConsolidate:" .. wowam.Colors.CYAN ..
                                   tostring(shouldConsolidate))
    ...
    for i = 1, 23 do
        ...
        if mainHandLink then
            ...
            if spell then
                ...
                DEFAULT_CHAT_FRAME:AddMessage(WowBee.Colors.RED .. "编号:" .. WowBee.Colors.LGREEN .. tostring(i) ..
                                                  WowBee.Colors.YELLOW .. "  名称:" .. WowBee.Colors.LGREEN .. spell ..
                                                  WowBee.Colors.YELLOW .. "  冷却时间:" .. WowBee.Colors.LGREEN ..
                                                  cd, 192, 0, 192, 0);
            end
        end
    end
...
function BeeShowSpellList(spell, index)
    ...
    for i = 1, index do
        ...
        if name == spell then
            WowBee_Message(WowBee.Colors.RED .. "(" .. k .. ") " .. WowBee.Colors.YELLOW .. name .. ", " ..
                               WowBee.Colors.LGREEN .. tostring(i))
...
function Bee_IsFailed(spell, unit)
    ...
    if WowBee.Spell.Failed then
        ...
        if aunid and WowBee.Spell.Failed[aunid] then
            if GetTime() - WowBee.Spell.Failed[aunid]["Time"] <= 1 then
    ...
    if WowBee.Spell.Delay then
        ...
        if tbl and unid then
            ...
            elseif tbl[unid] and tbl[unid]["Status"] and tbl[unid]["Status"] == "Star" and tbl[unid]["DelayTime"] then
                return false, "技能延時施放,施放中...";
            elseif tbl["All"] and tbl["All"]["Status"] and tbl["All"]["Status"] == "End" and tbl["All"]["EndTime"] and
                (GetTime() < tbl["All"]["EndTime"]) then
                return false, "技能延時中...";
            elseif tbl[unid] and tbl[unid]["Status"] and tbl[unid]["Status"] == "End" and tbl[unid]["EndTime"] and
                (GetTime() < tbl[unid]["EndTime"]) then
                return false, "技能延時中...";
            end
        end
    end
...
function Bee_IsItem(name, tunit, gcd, Special, isname, NOCD, typenumber, SpellLevel, temp_UnitGUID, unitguid,
    EnergyDemand)
    ...
    if WowBee.Spell.Property[name]["HasRange"] and tunit ~= "nogoal" then
        if not temp_UnitGUID then
            return false, typenumber,
                "需要个目标(如有问题请尝试用“无目标”(”normal”)参数或联系技术支持)";
        end
    end
    ...
    if not Isa then
        return false, typenumber,
            "不能对此目标施法(请尝试用“无目标”(”normal”)参数或联系技术支持)", itemCD;
    end
...
-- 判断技能是否可用
function Bee_IsSpell(name, tunit, gcd, Special, isname, NOCD, typenumber, SpellLevel, temp_UnitGUID, unitguid,
    EnergyDemand)

    if Bee_IsSpell_Conversion then
        local ASSC1, ASSC2, ASSC3, ASSC4, ASSC5, ASSC6 = Bee_IsSpell_Conversion(name, tunit, gcd, Special, isname,
            typenumber, SpellLevel, temp_UnitGUID, unitguid);
        if ASSC1 then
    ...
    if BeeSpellIsActivation then
        ...
        local temp_Activation, temp_Activation_1, temp_Activation_2, temp_Activation_3 = BeeSpellIsActivation(spellId,
            tunit, name);
    ...
    if WowBee.Spell.Property[name]["RaidSpell"] and tunit ~= "nogoal" then
        if WowBee.Spell.Property[name]["RaidSpell"] == 3 then
            if not (UnitPlayerOrPetInParty(tunit) or UnitPlayerOrPetInRaid(tunit) or UnitGUID(tunit) ==
                UnitGUID("player")) then
                return false, typenumber, "目标只能是小队或者团队";
            end
        elseif not (WowBee.Spell.Property[name]["RaidSpell"] == 2 and
            (UnitPlayerOrPetInParty(tunit) or UnitGUID(tunit) == UnitGUID("player"))) then
    ...
    if (not (BeeSpellIsMoveAll and BeeSpellIsMoveAll(spellId, tunit, name))) and
        (not (BeeSpellIsMove and BeeSpellIsMove(spellId, tunit, name))) then
        local T_temp1 = GetUnitSpeed("player")
        -- local T_temp2 = WowBee.Spell.Property[name]["CastTime"] --select(7,GetSpellInfo(name))
        local T_temp2 = select(4, GetSpellInfo(name))

        if T_temp1 and T_temp2 and T_temp1 > 0 and T_temp2 > 0 then
    ...
    if (WowBee.Spell.Property[name]["HasRange"] and tunit ~= "nogoal") then
        ...
        if UnitCanAssist("player", tunit) or UnitCanAttack("player", tunit) then
            ...
            elseif spellInRange == nil then
                return false, typenumber,
                    "不能对此目标施法(请尝试用“无目标”(”normal”)参数或联系技术支持)",
                    Cooldown;
            end
        else
    ...
    elseif not usable and not nomana then
        ...
        elseif (_Activation) then
            ...
            return false, typenumber, "该技能目前无法判断,请参考BeeIsRun第四参数或联系技术支持.",
                Cooldown;
        end
    elseif nomana then
        return false, typenumber, "能量不足", Cooldown;
...
function Bee_GetSpellInf(spell)
    ...
    if not spellId then
        skillType, spellId = "spell", BeeGetSpellIdEx(spell)
    end
...
function Bee_IsRunSpell(name, tunit, gcd, special, NOCD, EnergyDemand)
    ...
    if typenumber >= 1 and typenumber <= 3 and WowBee.Spell.Property[name]["Result"] and
        WowBee.Spell.Property[name]["Unitguid"] == unitguid then
    ...
    if typenumber == 1 then
        return Bee_IsSpell(name, tunit, gcd, special, isname, NOCD, typenumber, spellLevel, temp_UnitGUID, unitguid,
            EnergyDemand)
    elseif typenumber == 2 or typenumber == 3 then
        return Bee_IsItem(name, tunit, gcd, special, isname, NOCD, typenumber, spellLevel, temp_UnitGUID, unitguid,
            EnergyDemand)
    end
...
function BeeApiDecursive()
    if not Dcr then
        WowBee_Message(WowBee.Colors.RED .. "错误：" .. WowBee.Colors.LGREEN ..
                           "无法使用BeeApiDecursive()函数,需要安装或启动Decursive插件");
        return
    end
...
function BeeHolyPower(unit) -- 神圣能量
    return UnitPower(BeeUnit(unit, "player"), 9);
end

-- Enum.PowerType link:https://wowpedia.fandom.com/wiki/API_UnitPower
...
function BeeCancelUnitBuff(unit, buff)
...
function BeeSetFocus(unit, Name)
    ...
    if not Name or not unit or not mouseover then
        return false;
    end
...
function BeeAutoResume(Type, n, Buff, Spell, Battle)
...
function BeeUnitGetIncomingHeals(unit)
...
function BeeFindSpellItemInf(info1)
    ...
    if infoType == "item" then
        local spellId;
        local name, itemLink, itemRarity, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc,
            Texture, itemSellPrice;

        name, itemLink, itemRarity, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, Texture, itemSellPrice =
            GetItemInfo(info1);
        _, _, _, _, spellId, _, _, _, _, _, _, _, _, _ = string.find(itemLink,
...
function BeeUnitAuraFindText(unit, BuffName, index, FindText, Type)
    ...
    if (not Type) or (Type and Type == "buff") then
        for i = 1, MAX_TARGET_BUFFS do
            ...
            if (name == BuffName) then
                BeeUnitAuraGameTooltip = CreateFrame("GameTooltip", "BeeUnitAuraNumberGameTooltipFrame" .. "Tooltip",
...
function BeeSpellInRange(name, unit)
    unit = BeeUnit(unit, "target")
